    ...
    <section className="section-2 flex flex-col gap-[30px]">
      <div className="max-w-[1200px] flex items-end justify-between">
        ...
        <div className="flex items-end justify-end h-full mob:hidden">
          <div className="flex items-center gap-[50px]">
            <div
              className={`news-swiper-button-prev swiper-button-prev cursor-pointer swiper-control-btns ${atStart ? "disabled-button-class" : ""}`}
            >
              ←
            </div>
            <div
              className={`news-swiper-button-next swiper-button-next cursor-pointer swiper-control-btns ${atEnd ? "disabled-button-class" : ""}`}
            >
              →
            </div>
          </div>
      ...
      >
        {sliders.map((item, index) => (
          <SwiperSlide
            key={index}
            className="p-[20px] cases-swiper-slide rounded-[20px] bg-white shadow-md flex-col justify-between h-[calc(100% - 20px)]"
          >
              ...
              <div className="flex flex-col">
                <ul className="flex gap-[15px]">
                  {item.stats.map((stat, i) => (
                    <li
                      key={i}
                      className="relative pl-[22px] py-[5px] px-[10px] text-[#FB5D21] bg-[#fb5f2123] rounded-[4px] before:content-['•'] before:absolute before:left-[10px] before:text-[#FB5D21] before:text-[20px] before:top-1/2 before:-translate-y-1/2 mob:text-[11px]"
                    >
                      {stat}
                    </li>
                  ))}
