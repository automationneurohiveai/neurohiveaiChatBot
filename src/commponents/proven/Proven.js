        ...
        <section className="section">
            <div className="w-full flex flex-col items-center text-center">
                <span className="title-top-type">
                    ...
                    Proven Results</span>
                <h2 className="title-2 text-linear mt-[5px] max-w-[645px]">Here’s How the Automation Works for Real</h2>
                <p className="max-w-[450px] mt-[10px] text-[#818181] title-3">Real screenshots and data from the working automation — no mockups, no theory. Just live proof that it works.</p>
            </div>
            <div className="flex flex-col gap-[27px] mt-[30px]">
                {
                    provenData.map((item, index) => (
                        <div key={index} className='flex justify-between shadow-main rounded-[10px] bg-[#f1f1f1] p-[40px] gap-[85px] mob:flex-col-reverse mob:gap-[30px] mob:p-[30px]'>
                            ...
                            <div className='shadow-main mob:rounded-[12px]'>
                                <img src={`${process.env.PUBLIC_URL}${item.image}`} className='h-[216px]' />
                            </div>
                        </div>
                    ))
                }
