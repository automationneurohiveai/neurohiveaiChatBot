        ...
        <section className="section">
            <div className="w-full flex flex-col items-center text-center">
                <span className="title-top-type">
                    ...
                    Solution</span>
                <h2 className="title-2 text-linear mt-[5px] max-w-[582px]">From Manual Chaos to Streamlined Intelligence</h2>
                <p className="max-w-[436px] mt-[10px] text-[#818181] title-3">Using no-code tools and AI, we built a seamless system that eliminates manual invoice handling, reduces errors, and ensures every document is processed, logged, and archived — automatically.</p>
            </div>
            {isDesktop ? (
                <div className="flex flex-col shadow-main rounded-[10px] bg-[#f1f1f1] mt-[30px]">
                    ...
                    <div className="flex p-[15px] justify-between rounded-t-[10px] shadow-main px-[70px] items-center gap-[80px] pr-[60px]">
                        {
                            solutionData.map((item, index) => (
                                <div key={index} className={`cursor-pointer`} onClick={() => setActiveIndex(index)}>
                                    <p className={` ${acitveIndex === index ? 'title-3' : 'subtitle-16'} ${acitveIndex === index ? 'text-black' : 'text-[#818181]'}`}>{item.title}</p>
                                </div>
                            ))
                        }
                    </div>
                    <div className='p-[40px] gap-[100px] flex'>
                        <div className='flex flex-col gap-[10px] max-w-[352px]'>
                            ...
                            <p className='main-txt text-[#818181]'>{solutionData[acitveIndex].info}</p>
                        </div>
                        <div>
                            <img src={`${process.env.PUBLIC_URL}${solutionData[acitveIndex].image}`} />
                        </div>
                    </div>
                </div>
            ) : (
